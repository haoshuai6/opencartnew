...
class Cart {
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0) {
		...
		if (!$query->row['total']) {
			$this->db->query("INSERT " . DB_PREFIX . "cart SET customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', selected = '1', product_id = '" . (int)$product_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "', date_added = NOW()");
		} else {
		...
		$query_cart = $this->db->query("SELECT `cart_id` FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "'");
	}

	public function getCartId($product_id, $quantity = 1, $option = array(), $recurring_id = 0) {
		$cart_id = 0;

		$query = $this->db->query("SELECT cart_id FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "'");
		if ($query->num_rows) {
	...
	public function clear() {
		$this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND selected = '1' AND session_id = '" . $this->db->escape($this->session->getId()) . "'");
	}

	public function select($selects = array()) {
		$this->db->query("UPDATE " . DB_PREFIX . "cart SET selected = '0' WHERE customer_id = '" . (int)$this->customer->getId() . "'");
		foreach ($selects as $cart_id) {
			$this->db->query("UPDATE " . DB_PREFIX . "cart SET selected = '1' WHERE cart_id = '" . (int)$cart_id . "'");
		}
	}
