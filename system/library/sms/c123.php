...
class C123 {
	...
	public function __construct($controller) {
		$this->params = array(
			...
			'ac'=>$controller->config->get('config_c123_ac'),					                         //用户账号
			'authkey'=>$controller->config->get('config_c123_authkey'),	                             //认证密钥
			'cgid'=>$controller->config->get('config_c123_cgid'),    //通道组编号
			'm'=>'',		                                     //号码,多个号码用逗号隔开
			'c'=>'',  //iconv('gbk','utf-8',$c),		                 //如果页面是gbk编码，则转成utf-8编码，如果是页面是utf-8编码，则不需要转码
			'csid'=>trim($controller->config->get('config_c123_csid')),     //签名编号 ，可以为空，为空时使用系统默认的签名编号
			't'=>'', //$t                                              //定时发送，为空时表示立即发送
		);
	}
