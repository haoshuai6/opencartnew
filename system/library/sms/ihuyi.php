...
class Ihuyi {
	...
	public function __construct($controller) {
		$this->params = array(
			'account'  => $controller->config->get('config_c123_ac'),					                         //用户账号
			'password' => $controller->config->get('config_c123_authkey'),	                             //认证密钥
			'mobile'   => '',		                                     //号码,多个号码用逗号隔开
	...
	*/
	public function send() {
		...
		$this->log_result("发送消息：".iconv('utf-8','gbk//IGNORE',$this->params["content"]));
		
	  $res = $this->xml_to_array($re);
	  $res = $res['SubmitResult'];

		$this->log_result("发送结果：".iconv('utf-8','gbk//IGNORE',$res['msg']));   //发送成功返回的值
		if($res['code'] == 2){
	  	return true;
		}else{
	  	return false;
