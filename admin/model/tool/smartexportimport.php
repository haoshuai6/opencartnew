...
class ModelToolSmartExportImport extends Model {
	...
	private $keyids = array(
			'product' => array(),  //保存当前excel要导入的产品的product_id，以便在导入产品相关其他sheet的时候针对不在此数组中的记录进行忽略，以防止数据重复错误。
			'filtergroup' => array()  //保存当前excel要导入的filter Group的filter_group_id，以便在导入fiter sheet的时候针对不在此数组中的记录进行忽略，以防止导入错误数据。
	);
	...
	//取得所有的Filter Group的filter_group_id
