	...
	public function getCouponsByCustomer($customer = null, $view_expire = false) {
		...
		if(!empty($coupon_list)){
			if($view_expire) {
				...
				$coupon_query = $this->db->query("SELECT c.* FROM `" . DB_PREFIX . "coupon` AS c WHERE ((c.date_start = '0000-00-00' OR c.date_start < NOW()) AND (c.date_end = '0000-00-00' OR c.date_end > NOW())) AND c.status = '1' AND c.coupon_id IN (".implode(",",$coupon_list).")");
			}
