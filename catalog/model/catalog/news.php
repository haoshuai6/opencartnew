...
class ModelCatalogNews extends Model {
	...
	public function getNews($data = array()) {
		$sql = "SELECT n.news_id FROM " . DB_PREFIX . "sb_news n LEFT JOIN " . DB_PREFIX . "sb_news_description nd ON (n.news_id = nd.news_id) LEFT JOIN " . DB_PREFIX . "sb_news_to_store n2s ON (n.news_id = n2s.news_id) LEFT JOIN " . DB_PREFIX . "sb_nauthor nau ON (n.nauthor_id = nau.nauthor_id) ";
	...
	public function getTotalNews($data = array()) {
		$sql = "SELECT COUNT(DISTINCT n.news_id) AS total FROM " . DB_PREFIX . "sb_news n LEFT JOIN " . DB_PREFIX . "sb_news_description nd ON (n.news_id = nd.news_id) LEFT JOIN " . DB_PREFIX . "sb_news_to_store n2s ON (n.news_id = n2s.news_id) LEFT JOIN " . DB_PREFIX . "sb_nauthor nau ON (n.nauthor_id = nau.nauthor_id) ";
		if (!empty($data['filter_ncategory_id'])) {
			$sql .= " LEFT JOIN " . DB_PREFIX . "sb_news_to_ncategory n2n ON (n.news_id = n2n.news_id)";			
		}
		$sql .= " WHERE nd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND n.status = '1' AND n2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND n.date_pub < NOW()";	
	...
	public function getArchive() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "sb_news_archive WHERE store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY year DESC");
