...
class ModelCatalogncategory extends Model {
	...
	public function getTotalncategoriesByncategoryId($parent_id = 0) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "sb_ncategory c LEFT JOIN " . DB_PREFIX . "sb_ncategory_to_store c2s ON (c.ncategory_id = c2s.ncategory_id) WHERE c.parent_id = '" . (int)$parent_id . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND c.status = '1'");
